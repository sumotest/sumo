...
MSLCM_LC2013::informLeader(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
	...
	if(DEBUG_COND){
        ...
        if (dv < 0
            ...
            // using brakeGap() without headway seems adequate in a situation where the obstacle (the lane end) is not moving [XXX implemented in branch ticket860, can be used in general if desired] (Leo).
            ...
            // not enough time to overtake?        (skipped for a stopped leader [currently only for ballistic update XXX: check if appropriate for euler, too] to ensure that it can be overtaken if only enough space is exists) (Leo)
            ...
            return -1;  // XXX: using -1 is ambiguous for the ballistic update! Currently this is being catched in patchSpeed() (Leo), consider returning std::numeric_limits...max()
        }
...
MSLCM_LC2013::informFollower(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
    ...
    if ((blocked & LCA_BLOCKED_BY_FOLLOWER) != 0) {
        ...
        if (decelGap > 0 && decelGap >= secureGap) {
        	...
        	//      this seems to be supposed in the following (euler code)...?! (Leo)
            ...
            if(MSGlobals::gSemiImplicitEulerUpdate){
                ...
                // XXX: This block should actually do as well for euler update (TODO: test!)
...
MSLCM_LC2013::_wantsChange(
    ...
    // which are part of the roundabout. refs #1807 (Leo)
...
//#endif
    ...
    //      in the ballistic branch (ticket860). Both variants may be combined in future.
    if(MSGlobals::gSemiImplicitEulerUpdate){
...
//    // Debug (Leo)
//    std::cout << "\ncalled distanceAlongNextRoundabout():\n"
//            << "position = " << position
//            << " initialLane = " << (initialLane != 0 ? initialLane->getID() : "NULL")
//            << " continuationLanes = " << toString(continuationLanes) << std::endl;
...
//            // Debug (Leo)
//            std::cout << "'"<< lane->getID() << "' is a roundabout lane!" << std::endl;
            ...
            if (it+1 != continuationLanes.end() && *(it+1) != 0 && (*(it+1))->getEdge().isRoundabout()){
                ...
                //                    // Debug (Leo)
                //                    std::cout << "has internal length behind: " << std::endl
                //                            << linkLength  << std::endl;
            }
